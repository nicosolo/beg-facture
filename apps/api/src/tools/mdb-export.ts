...
export async function exportMdbToJson(mdbPath: string, outputDir: string): Promise<void> {
                        ...
                        if (stdout && stdout.trim().length > 0) {
                            ...
                            if (firstLine) {
                                try {
                                    ...
                                    exportSuccess = true
                                    console.log(`✓ Export succeeded for ${table} using ${attempt.name}`)
                                    break
                                } catch {
    ...
    if (failedTables.length > 0) {
        console.warn(`⚠️  Failed to export ${failedTables.length} tables: ${failedTables.join(', ')}`)
    }
    
    console.log(`✓ Export completed. Successfully exported ${tables.length - failedTables.length} out of ${tables.length} tables`)
}