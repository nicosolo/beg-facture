...
export function getYearRange(referenceDate: Date = new Date()): DateRangeInterval {
  const year = referenceDate.getFullYear()
...
export function shiftWeekRange(currentFrom: Date | undefined, direction: number): DateRangeInterval {
  const base = currentFrom ? new Date(currentFrom) : new Date()
  base.setDate(base.getDate() + 7 * direction)
  return getWeekRange(base)
}

export function shiftMonthRange(currentFrom: Date | undefined, direction: number): DateRangeInterval {
  const base = currentFrom ? new Date(currentFrom) : new Date()
...
export function shiftYearRange(currentFrom: Date | undefined, direction: number): DateRangeInterval {
  const base = currentFrom ? new Date(currentFrom) : new Date()
